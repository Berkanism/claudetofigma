(() => {
    ...
    return __async(this, null, function* () {
      ...
      try {
        ...
        if (!collectionId && params.variableKey) {
          const variable = yield figma.variables.importVariableByKeyAsync(params.variableKey);
          collectionId = variable.variableCollectionId;
        }
        if (!collectionId) {
          sendResponse(id, void 0, `collectionId or variableKey required`);
          return;
        }
        const modeId = params.modeId;
        if (!modeId) {
          sendResponse(id, void 0, `modeId required`);
          return;
        }
        const collection = figma.variables.getVariableCollectionById(collectionId);
        if (!collection) {
          sendResponse(id, void 0, `VariableCollection not found for id: ${collectionId}`);
          return;
        }
  ...
  function handleDebugVariableApi(id, params) {
    return __async(this, null, function* () {
      try {
        ...
        if (params.variableKey) {
          const v = yield figma.variables.importVariableByKeyAsync(params.variableKey);
          importedInfo = {
            id: v.id,
            name: v.name,
